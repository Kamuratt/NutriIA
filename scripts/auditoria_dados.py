# auditoria_dados.py (VERSÃO FINAL E ROBUSTA)
import sqlite3
import pandas as pd
import unicodedata
import random
        ...
        if total_calculado > 0:
            ...
            for receita_id, titulo in receitas_calculadas:
                ...
                for (nome,) in ingredientes:
                    # CORREÇÃO 1: Garante que 'nome' não seja nulo antes de qualquer operação
                    if not nome:
                        continue
            ...
            if qualidade['ruim'] or ingredientes_nao_encontrados:
                ...
                if ingredientes_nao_encontrados:
                    ...
                    # CORREÇÃO 2: Filtra a lista final para garantir que nenhum item nulo seja processado
                    itens_validos = [item for item in sorted(list(ingredientes_nao_encontrados)) if item]
                    for item in itens_validos[:15]:
                        print(f'   - "{item.lower()}": "???"')
