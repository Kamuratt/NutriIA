...
def carregar_alimentos_conhecidos(engine: Engine) -> set:
    conhecidos = set()
    try:
        # Tenta carregar TACO CSV se existir (opcional)
        try:
        ...
        print(f"AVISO: Erro ao carregar alimentos conhecidos: {e}")
        return conhecidos
...
# --- Aplicação e Limpeza (A Correção Principal) ---

def aplicar_correcoes_e_limpeza(engine: Engine, mapa_de_correcoes: dict) -> int:
    """
    Varre as receitas, corrige os nomes, remove IGNORE e RESETA FLAGS DE CÁLCULO.
    """
    ...
    # Query preparada para resetar flags
    sql_update = text("""
        UPDATE receitas 
        SET 
    ...
    for i, (receita_id, ingredientes_json) in enumerate(receitas):
        ...
        # Se o JSON vier como string (alguns drivers do SQL fazem isso), converte
        if isinstance(ingredientes_json, str):
            try: ingredientes_json = json.loads(ingredientes_json)
            except: continue
        ...
        for ing in ingredientes_json:
            ...
            if mapa_de_correcoes.get(nome_original) == "IGNORE":
                modificado = True
                continue # PULA este ingrediente
        ...
        # Se a lista mudou de tamanho (removido IGNORE) ou conteúdo (nome corrigido)
        if modificado or len(ingredientes_finais) != len(ingredientes_json):
            with engine.begin() as conn:
                conn.execute(sql_update, {"ingredientes_json": json.dumps(ingredientes_finais, ensure_ascii=False), "id": receita_id})
            receitas_modificadas += 1
            
    print(f"Limpeza concluída! {receitas_modificadas} receitas foram corrigidas e marcadas para recálculo.")
...
def gerar_relatorio_final(engine: Engine, stats: dict):
    ...
    with engine.connect() as conn:
        ...
        print(f"  - Receitas com Ingredientes Estruturados: {llm}")
        print(f"  - Receitas com Nutrientes Calculados:   {calc}")
        
        print("\n--- [2. DIAGNÓSTICO DE QUALIDADE] ---")
        total_unicos = stats.get('total_ingredientes_unicos', 0)
        conhecidos = stats.get('ingredientes_ja_conhecidos', 0)
        ...
        print(f"Novas Correções Aprendidas com a IA: {stats.get('correcoes_aprendidas_nesta_execucao', 0)}")
        print(f"Receitas Modificadas e Resetadas:    {stats.get('receitas_modificadas', 0)}")
        
        print("\n[!] AÇÃO RECOMENDADA:")
        print(f"    Agora existem {stats.get('receitas_modificadas', 0)} receitas prontas para serem calculadas.")
        print("    Rode o script 'calcular_nutrientes.py' para processá-las.")
    print("\n" + "="*80)
