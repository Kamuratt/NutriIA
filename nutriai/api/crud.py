...
def get_receitas_nutricionais_sample(db: Session, limit: int = 50):
    """Busca uma amostra aleatória de receitas que JÁ TÊM nutrientes calculados."""
    return db.query(models.Receita).filter(models.Receita.nutrientes_calculados == True).order_by(func.random()).limit(limit).all()
